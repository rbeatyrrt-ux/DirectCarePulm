...
app.post('/api/clinics', verifyToken, async (req, res) => {
  ...
  const { 
    clinic_name, billing_email, phone_number, authorized_rep_email, 
    address, baa_signer_name, baa_signer_title, baa_signature, admin_signature
  } = req.body;
  ...
  try {
    const query = `
      INSERT INTO clinics (
        clinic_name, billing_email, phone_number, authorized_rep_email, 
        address, baa_signer_name, baa_signer_title, baa_signature, admin_signature, baa_signed_date
      )
      VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, CURRENT_TIMESTAMP)
      ON CONFLICT (clinic_name) DO UPDATE SET 
        billing_email = EXCLUDED.billing_email, phone_number = EXCLUDED.phone_number,
        authorized_rep_email = EXCLUDED.authorized_rep_email, address = EXCLUDED.address,
        baa_signer_name = EXCLUDED.baa_signer_name, baa_signer_title = EXCLUDED.baa_signer_title,
        baa_signature = EXCLUDED.baa_signature, admin_signature = EXCLUDED.admin_signature,
        baa_signed_date = CURRENT_TIMESTAMP
      RETURNING *;
    `;
    const result = await pool.query(query, [clinic_name, billing_email, phone_number || null, authorized_rep_email || null, address, baa_signer_name, baa_signer_title, baa_signature, admin_signature]);
...
app.put('/api/clinics/:id', verifyToken, async (req, res) => {
  ...
  } catch (err) { res.status(500).json({ error: 'Failed to update clinic: ' + err.message }); }
});
...
app.delete('/api/clinics/:id', verifyToken, async (req, res) => {
  ...
  } catch (err) { res.status(500).json({ error: 'Failed to delete clinic' }); }
});

app.post('/api/auth/sign-baa', verifyToken, async (req, res) => {
  try {
    const { signer_name } = req.body;
    if (!signer_name || !signer_name.trim()) return res.status(400).json({ error: 'Legal signer name is required to execute the BAA.' });

    const ipAddress = req.headers['x-forwarded-for'] || req.socket.remoteAddress || 'Unknown';
    const updateResult = await pool.query(
      `UPDATE users SET baa_signed = TRUE, baa_signed_date = CURRENT_TIMESTAMP, baa_signer_name = $1, baa_ip_address = $2
       WHERE user_id = $3 RETURNING user_id, full_name, email, role, clinic_name, credentials, npi, must_change_password, baa_signed, baa_signer_name, baa_signed_date, baa_ip_address`,
  ...
  } catch (err) { res.status(500).json({ error: 'Failed to record BAA signature: ' + err.message }); }
});
...
app.get('/api/auth/signed-baa-pdf', verifyToken, async (req, res) => {
  try {
    ...
    if (!userData.baa_signed) return res.status(400).json({ error: 'BAA has not yet been executed for this account.' });
    ...
    doc.end();
  } catch (err) { res.status(500).json({ error: 'Failed to generate executed BAA PDF' }); }
});
...
app.get('/api/clinics/:id/baa-pdf', verifyToken, async (req, res) => {
  ...
  try {
    ...
    doc.moveDown(1.5);
    doc.fontSize(9).fillColor('#2d3748').font('Helvetica').text(`This HIPAA Business Associate Agreement (“Agreement”) is entered into as of ${effectiveDate} (“Effective Date”) by and between ${clinic.clinic_name}, a participating healthcare entity (“Covered Entity”), and DirectCare Pulmonary Diagnostics LLC, an Ohio limited liability company (“Business Associate”).`, { align: 'justify', lineGap: 3 });
    doc.moveDown(1);
    doc.font('Helvetica-Bold').text('RECITALS');
    doc.font('Helvetica').text('WHEREAS, Business Associate provides certain services to Covered Entity that involve the use and/or disclosure of Protected Health Information (“PHI”);\nWHEREAS, Covered Entity is a “covered entity” as defined in the Health Insurance Portability and Accountability Act of 1996 (“HIPAA”) and the regulations promulgated thereunder;\nWHEREAS, Business Associate is a “business associate” as defined in HIPAA and the HIPAA Rules (as defined below); and\nWHEREAS, Covered Entity is required under HIPAA to obtain satisfactory assurances that Business Associate will appropriately safeguard PHI it receives, creates, maintains, or transmits on behalf of Covered Entity.\nNOW, THEREFORE, in consideration of the mutual promises set forth herein and other good and valuable consideration, the receipt and sufficiency of which are hereby acknowledged, the parties agree as follows:', { align: 'justify', lineGap: 3 });
    doc.moveDown(1);
    doc.font('Helvetica-Bold').text('1. Definitions');
    doc.font('Helvetica').text('1.1 “HIPAA Rules” means, collectively, the Privacy Rule, Security Rule, Breach Notification Rule, and Enforcement Rule at 45 C.F.R. Parts 160 and 164.\n1.2 “Business Associate” has the meaning set forth at 45 C.F.R. § 160.103 and, for purposes of this Agreement, refers to the entity identified above as Business Associate.\n1.3 “Covered Entity” has the meaning set forth at 45 C.F.R. § 160.103 and, for purposes of this Agreement, refers to the entity identified above as Covered Entity.\n1.4 “Protected Health Information” or “PHI” has the meaning set forth at 45 C.F.R. § 160.103, and for purposes of this Agreement includes all individually identifiable health information created, received, maintained, or transmitted by Business Associate on behalf of Covered Entity, in any form or medium.\n1.5 “Electronic Protected Health Information” or “ePHI” means PHI that is transmitted or maintained in electronic media, as defined at 45 C.F.R. § 160.103.\n1.6 “Breach” has the meaning set forth at 45 C.F.R. § 164.402.\n1.7 “Unsecured PHI” has the meaning set forth at 45 C.F.R. § 164.402.\n1.8 “Security Incident” has the meaning set forth at 45 C.F.R. § 164.304.\n1.9 “Secretary” means the Secretary of HHS or the Secretary’s designee.\n1.10 Capitalized terms used but not otherwise defined in this Agreement shall have the meanings given to them in the HIPAA Rules.', { align: 'justify', lineGap: 3 });
    doc.moveDown(1);
    doc.font('Helvetica-Bold').text('2. Obligations and Activities of Business Associate');
    doc.font('Helvetica').text('2.1 Permitted Use and Disclosure. Business Associate shall not use or disclose PHI other than as permitted or required by this Agreement, as permitted or required by applicable law, or as otherwise authorized in writing by Covered Entity.\n2.2 Safeguards. Business Associate shall use appropriate administrative, physical, and technical safeguards, including a risk analysis and risk management program, access controls, and workforce security measures, to prevent the use or disclosure of PHI other than as provided for by this Agreement.\n2.3 Mitigation. Business Associate shall mitigate, to the extent practicable, any harmful effect that is known to Business Associate of a use or disclosure of PHI by Business Associate or its employees, agents, or subcontractors in violation of this Agreement or the HIPAA Rules.\n2.4 Reporting of Breaches and Security Incidents. Business Associate shall report to Covered Entity any Breach of Unsecured PHI in accordance with 45 C.F.R. § 164.410 and any Security Incident that results in unauthorized access, use, or disclosure of PHI. Such report shall be made without unreasonable delay and in no case later than 15 calendar days after discovery of the Breach or Security Incident.\n2.5 Subcontractors and Agents. Business Associate shall ensure that any subcontractor, agent, or other third party to whom it provides PHI on behalf of Covered Entity agrees in writing to the same restrictions, conditions, and requirements that apply to Business Associate.\n2.6 Access to PHI. To the extent Business Associate maintains PHI in a Designated Record Set, Business Associate shall make such PHI available to Covered Entity.\n2.7 Amendment of PHI. To the extent Business Associate maintains PHI in a Designated Record Set, Business Associate shall make such PHI available for amendment and shall incorporate any amendments to PHI as directed by Covered Entity.\n2.8 Accounting of Disclosures. Business Associate shall maintain and, within a reasonable time following Covered Entity’s written request, provide to Covered Entity such information as is necessary to permit Covered Entity to provide an accounting of disclosures of PHI.\n2.9 Internal Practices, Books, and Records. Business Associate shall make its internal practices, books, and records available to the Secretary for purposes of determining Covered Entity’s compliance with the HIPAA Rules.\n2.10 Compliance with Law. Business Associate shall comply with the HIPAA Rules and any other applicable federal or state laws and regulations governing the privacy or security of PHI.', { align: 'justify', lineGap: 3 });
    doc.addPage();
    doc.font('Helvetica-Bold').text('3. Workforce Training and Security Awareness');
    doc.font('Helvetica').text('3.1 HIPAA Privacy Training. Business Associate shall provide training on the requirements of the HIPAA Privacy Rule and on Business Associate’s related policies and procedures to all members of its workforce who create, receive, maintain, or transmit PHI on behalf of Business Associate.\n3.2 Security Awareness and Training. Business Associate shall implement a security awareness and training program for all members of its workforce, including management, in accordance with 45 C.F.R. § 164.308(a)(5) of the HIPAA Security Rule.', { align: 'justify', lineGap: 3 });
    doc.moveDown(1);
    doc.font('Helvetica-Bold').text('4. Permitted Uses and Disclosures by Business Associate');
    doc.font('Helvetica').text('4.1 Services for Covered Entity. Except as otherwise limited by this Agreement or applicable law, Business Associate may use or disclose PHI only as necessary to perform the services set forth in the underlying agreement(s) between Covered Entity and Business Associate.\n4.2 Use for Proper Management and Administration. Business Associate may use PHI for its proper management and administration or to carry out its legal responsibilities, provided that such use is permitted by the HIPAA Rules and applicable law.\n4.3 Disclosures for Proper Management and Administration. Business Associate may disclose PHI for its proper management and administration or to carry out its legal responsibilities, provided that (a) the disclosures are required by law, or (b) Business Associate obtains reasonable assurances from the person to whom the PHI is disclosed that the PHI will be held confidentially.\n4.4 De-identified Information. Business Associate may de-identify PHI in accordance with 45 C.F.R. § 164.514(a)–(c).\n4.5 Prohibited Uses and Disclosures. Business Associate shall not sell PHI or use PHI for marketing or fundraising purposes in a manner that would violate the HIPAA Rules or other applicable law.', { align: 'justify', lineGap: 3 });
    doc.moveDown(1);
    doc.font('Helvetica-Bold').text('5. Term and Termination');
    doc.font('Helvetica').text('5.1 Term. This Agreement shall become effective as of the Effective Date and shall remain in effect until terminated in accordance with this Section 5.\n5.2 Termination for Cause. Covered Entity may terminate this Agreement and any related services agreement(s) immediately if it determines that Business Associate has materially breached this Agreement.\n5.3 Other Termination Rights. Business Associate may terminate this Agreement upon written notice to Covered Entity if Business Associate reasonably determines that continuing to perform under this Agreement would cause Business Associate to violate the HIPAA Rules.\n5.4 Obligations of Business Associate Upon Termination. Upon termination or expiration of this Agreement for any reason, Business Associate shall return to Covered Entity or, if agreed to by Covered Entity, destroy all remaining PHI.\n5.5 Infeasibility of Return or Destruction. If Business Associate determines that returning or destroying PHI is infeasible, Business Associate shall extend the protections of this Agreement to such PHI and limit further uses and disclosures.\n5.6 Reporting to HHS. If Covered Entity determines that termination of this Agreement is not feasible, Covered Entity shall report the violation to the Secretary.', { align: 'justify', lineGap: 3 });
    doc.addPage();
    doc.font('Helvetica-Bold').text('6. Miscellaneous');
    doc.font('Helvetica').text('6.1 Amendment. The parties agree to take such action as is necessary to amend this Agreement from time to time as may be required to comply with the requirements of HIPAA.\n6.2 Survival. The respective rights and obligations of Business Associate and Covered Entity under this Agreement that, by their nature, are intended to survive termination or expiration of this Agreement shall survive such termination or expiration.\n6.3 Interpretation. Any ambiguity in this Agreement shall be resolved to permit compliance with the HIPAA Rules.\n6.4 Governing Law. This Agreement shall be governed by and construed in accordance with the laws of the State of Ohio, without regard to its conflict-of-law principles.\n6.5 Indemnification. Business Associate shall indemnify, defend, and hold harmless Covered Entity and its directors, officers, employees, and agents from and against any and all claims arising out of or relating to Business Associate’s breach of this Agreement.\n6.6 Entire Agreement. This Agreement constitutes the entire agreement between the parties with respect to the subject matter hereof.\n6.7 Counterparts. This Agreement may be executed in counterparts, each of which shall be deemed an original.', { align: 'justify', lineGap: 3 });
    doc.moveDown(2);
    ...
    doc.end();
  } catch (err) { res.status(500).json({ error: 'Failed to generate PDF package' }); }
});
...
app.put('/api/auth/update-credentials', verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (credentials !== undefined) { query += `, credentials = $${paramIndex++}`; params.push(credentials); }
    if (npi !== undefined) { query += `, npi = $${paramIndex++}`; params.push(npi); }
...
app.post('/api/clinic-schedules', verifyToken, async (req, res) => {
  ...
  } catch (err) { res.status(500).json({ error: 'Failed to save clinic schedule: ' + err.message }); }
});

app.get('/api/booked-slots', verifyToken, async (req, res) => {
  const { clinic_name, date } = req.query;
  if (!clinic_name || !date) return res.status(400).json({ error: 'Clinic name and date are required.' });

  try {
    const result = await pool.query(
      `SELECT time_block FROM service_requests WHERE clinic_name = $1 AND requested_date::date = $2::date AND is_deleted = FALSE`,
      [clinic_name, date]
    );
    res.json(result.rows.map(row => row.time_block));
...
app.put('/api/users/:id', verifyToken, async (req, res) => {
  ...
  try {
    ...
    let params = [full_name, email, role, ['admin', 'rrt', 'physician', 'billing'].includes(role) ? null : clinic_name, credentials, npi];
  ...
  } catch (err) { res.status(500).json({ error: 'Failed to update user: ' + err.message }); }
});
