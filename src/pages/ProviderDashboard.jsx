...
export default function ProviderDashboard({ token, user, onLogout }) {
  ...
  // Modal State for Signing Pending Orders
  ...
  const handleCreateOrder = async (e) => {
    ...
    if (!signaturePin) {
      alert('Please enter your Signature PIN to legally authorize and sign this order.');
      return;
    }
    ...
    try {
      ...
      setSuccessMsg('PFT order signed and submitted successfully!');
      setPatientName('');
      setPatientDob(''); 
  ...
  const handleSignPendingOrder = async (e) => {
    e.preventDefault();
    if (!signingRequest) return;
    if (!modalPin) {
      alert('Please enter your Signature PIN to sign this order.');
      return;
    }
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Failed to sign order');

      alert('Order successfully signed and sent to scheduling queue!');
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#f4f6f9', fontFamily: 'Segoe UI, Tahoma, Geneva, Verdana, sans-serif' }}>
      ...
      <header style={{ backgroundColor: '#1a2a47', paddingTop: '20px' }}>
        <div style={{ padding: '0 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          <div>
            <h1 style={{ margin: 0, fontSize: '22px', color: 'white' }}>DirectCare Provider Portal</h1>
            <span style={{ fontSize: '13px', color: '#a0aec0' }}>Clinic: <strong>{user?.clinic_name || 'Independent Clinic'}</strong> | Provider: {user?.full_name}</span>
          </div>
        ...
        {activeTab === 'account' ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 1.3fr', gap: '30px', alignItems: 'start' }}>
            ...
            <div style={{ backgroundColor: 'white', borderRadius: '8px', padding: '30px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
              <h2 style={{ marginTop: 0, color: '#1a2a47', fontSize: '18px', marginBottom: '20px' }}>Submit & Sign New PFT Order</h2>
              ...
              <form onSubmit={handleCreateOrder} autoComplete="off">
                ...
                <div style={{ backgroundColor: '#fffaf0', border: '1px solid #feebc8', borderRadius: '6px', padding: '15px', marginBottom: '20px' }}>
                  <h3 style={{ margin: '0 0 8px 0', fontSize: '13px', color: '#c05621', fontWeight: 'bold' }}>Legal Electronic Signature & Binding Attestation</h3>
                  <p style={{ fontSize: '12px', color: '#744210', lineHeight: '1.4', margin: '0 0 12px 0' }}>
                    By entering your Signature PIN below, you certify under clinical and legal authority that the selected diagnostic tests and MDI instructions are medically necessary for this patient, and this electronic submission serves as a legally binding medical order.
                  </p>
                  <div>
                    <label style={{ display: 'block', marginBottom: '5px', fontSize: '13px', fontWeight: 'bold', color: '#2d3748' }}>Enter Signature PIN</label>
                    <input type="password" name="dc_unique_signature_pin" placeholder="Enter signature PIN..." value={signaturePin} onChange={e => setSignaturePin(e.target.value)} autoComplete="new-password" style={{ width: '100%', padding: '10px', borderRadius: '4px', border: '1px solid #cbd5e0', boxSizing: 'border-box', backgroundColor: 'white' }} required />
                  </div>
                </div>
                ...
                <button type="submit" disabled={submitting || allowedDates.length === 0} style={{ width: '100%', padding: '12px', backgroundColor: (submitting || allowedDates.length === 0) ? '#cbd5e0' : '#1a2a47', color: 'white', border: 'none', borderRadius: '4px', fontWeight: 'bold', cursor: (submitting || allowedDates.length === 0) ? 'not-allowed' : 'pointer' }}>
                  {submitting ? 'Signing & Submitting Order...' : 'Sign & Submit Order'}
                </button>
              </form>
            </div>
                          ...
                          return (
                            <tr key={req.request_id} style={{ borderBottom: '1px solid #eee', fontSize: '13px' }}>
                              ...
                              <td style={{ padding: '10px', textAlign: 'center', whiteSpace: 'nowrap' }}>
                                {req.status === 'PENDING_PROVIDER_SIGNATURE' && (
                                  <button onClick={() => { setSigningRequest(req); setModalPin(''); }} style={{ padding: '5px 10px', backgroundColor: '#5b21b6', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px', fontWeight: 'bold', marginRight: '5px' }}>
                                    Review & Sign Order
                                  </button>
                                )}
                                ...
                                {req.status === 'COMPLETED' ? (
                                  ...
                                  req.status !== 'PRELIMINARY_RESULTS' && req.status !== 'PENDING_PROVIDER_SIGNATURE' && <span style={{ fontSize: '11px', color: '#888', fontStyle: 'italic' }}>—</span>
                                )}
                              </td>
                            </tr>
                          );
      ...
      {signingRequest && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 9999 }}>
          <div style={{ backgroundColor: 'white', padding: '35px', borderRadius: '8px', width: '550px', boxShadow: '0 4px 20px rgba(0,0,0,0.2)' }}>
            <h2 style={{ marginTop: 0, color: '#1a2a47', fontSize: '18px' }}>Sign Pending Patient Order</h2>
            <p style={{ fontSize: '13px', color: '#4a5568', lineHeight: '1.4' }}>
              You are signing order for patient <strong>{signingRequest.patient_name}</strong>.
            </p>
            <div style={{ backgroundColor: '#f8f9fa', padding: '12px', borderRadius: '6px', fontSize: '12px', marginBottom: '20px' }}>
              <p style={{ margin: '0 0 5px 0' }}><strong>Tests:</strong> {signingRequest.tests_ordered}</p>
              <p style={{ margin: 0 }}><strong>Indication:</strong> {signingRequest.ordering_reason}</p>
            </div>

            <form onSubmit={handleSignPendingOrder}>
              <div style={{ marginBottom: '20px' }}>
                <label style={{ display: 'block', marginBottom: '5px', fontSize: '13px', fontWeight: 'bold', color: '#333' }}>Enter Signature PIN</label>
                <input 
                  type="password" 
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                ...
                <button type="button" onClick={() => setSigningRequest(null)} style={{ padding: '12px 20px', backgroundColor: '#6c757d', color: 'white', border: 'none', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer' }}>
                  Cancel
                </button>
              </div>
            </form>
