...
const pool = new Pool({
  host: 'directcare-pft-db.cs5m8662wh1z.us-east-1.rds.amazonaws.com',
  database: 'postgres',
  user: 'postgres',
  password: "sVE1['ag4G",
  port: 5432,
...
// PRELIMINARY ROUTE: Upload PDF buffer directly to Amazon S3
  ...
  try {
    ...
    if (req.file) {
      ...
      await s3Client.send(new PutObjectCommand({
        ...
        ServerSideEncryption: 'AES256' // Enforce HIPAA encryption at rest
      }));
...
app.get('/api/requests/:id/raw-report', verifyToken, async (req, res) => {
  try {
    ...
    const s3Uri = reqResult.rows[0].uploaded_report_path; // Format: s3://bucket-name/key
...
// MULTI-PAGE CLINICAL & BILLING REPORT PACKAGE PDF GENERATOR
app.get('/api/requests/:id/pdf', verifyToken, async (req, res) => {
  try {
    ...
    // ==========================================
    // PAGE 1: CONFIDENTIAL COVER SHEET (NO PHI)
    // ==========================================
    doc.addPage();
    ...
    // ==========================================
    // PAGE 2: ORIGINAL PROVIDER ORDER & SIGNATURE
    // ==========================================
    doc.addPage();
    ...
    // Provider Electronic Signature Block on Page 2
    ...
    // ==========================================
    // PAGE 3: CPT BILLING & MODIFIERS GUIDE
    // ==========================================
    doc.addPage();
    ...
    // ==========================================
    // PAGE 4: PHYSICIAN READ REPORT & SIGN-OFF
    // ==========================================
    doc.addPage();
    ...
    // ==========================================
    // PAGE 5: ATTACHED PFT DIAGNOSTIC REPORT
    // ==========================================
    doc.addPage();
